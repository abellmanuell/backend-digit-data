...
const { getRefreshToken } = require("../../config/factory/refresh_token_factory");
...
router.post("/",body("refresh_token").notEmpty().escape() ,async (req, res)=>{
    const result = validationResult(req);
    if(result.isEmpty()){
        const {refresh_token: id} = matchedData(req)
        const {userId, ...other} = await getRefreshToken(id)
        ...
        return server_response(200, res, "Successfully verified!", {
            token, refresh_token: await refreshToken(userId)
        });
    }
